...
import { isWorkoutEmotionalState, type WorkoutEmotionalState } from "@/lib/workout-session-feedback";
...
type IncomingExercise = { id: string; routineExerciseId: string; catalogExerciseId: string | null; position: number; name: string; muscle: string; sets: IncomingSet[] };
type IncomingSession = { id: string; routineId: string; status: "IN_PROGRESS" | "FINISHED"; startedAt: Date; finishedAt: Date | null; durationSeconds: number | null; notes: string | null; emotionalRating: number | null; emotionalState: WorkoutEmotionalState | null; version: number; updatedAt: Date; clientUpdatedAt: Date; exercises: IncomingExercise[] };
...
function readSession(body: unknown): IncomingSession | null {
  ...
  const emotionalState = raw.emotionalState === null || raw.emotionalState === undefined ? null : isWorkoutEmotionalState(raw.emotionalState) ? raw.emotionalState : null;
  const notes = nullableText(raw.notes, 2_000);
  if (!isUuid(String(body.operationId || "")) || !isUuid(id) || !isUuid(routineId) || (status !== "IN_PROGRESS" && status !== "FINISHED") || !startedAt || !updatedAt || !clientUpdatedAt || version === null || (raw.finishedAt !== null && !finishedAt) || (status === "FINISHED" && !finishedAt) || durationSeconds === null && raw.durationSeconds !== null && raw.durationSeconds !== undefined || emotionalRating === null && raw.emotionalRating !== null && raw.emotionalRating !== undefined || emotionalState === null && raw.emotionalState !== null && raw.emotionalState !== undefined || !Array.isArray(raw.exercises) || !raw.exercises.length || raw.exercises.length > 60) return null;
  ...
  for (const [position, item] of raw.exercises.entries()) {
    ...
    const catalogExerciseId = nullableText(item.catalogExerciseId, 100);
    exercises.push({ id: String(item.id), routineExerciseId: String(item.routineExerciseId), catalogExerciseId, position, name: String(item.name).trim(), muscle: String(item.muscle).trim(), sets });
  }
  return { id, routineId, status, startedAt, finishedAt, durationSeconds, notes, emotionalRating, emotionalState, version, updatedAt, clientUpdatedAt, exercises };
}
...
export async function POST(request: Request) {
  ...
  await prisma.$transaction(async (tx) => {
    const data = { status: session.status, startedAt: session.startedAt, finishedAt: session.finishedAt, durationSeconds: session.durationSeconds, notes: session.notes, emotionalRating: session.emotionalRating, emotionalState: session.emotionalState, version: session.version, clientUpdatedAt: session.clientUpdatedAt };
    ...
    await tx.workoutSessionExercise.createMany({ data: session.exercises.map((exercise) => ({ id: exercise.id, sessionId: session.id, routineExerciseId: exercise.routineExerciseId, catalogExerciseId: exercise.catalogExerciseId, position: exercise.position, name: exercise.name, muscle: exercise.muscle })) });
