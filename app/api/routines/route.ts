...
function readRoutine(body: unknown) {
  ...
  const exercises = sourceExercises.map((source, position) => {
    ...
    const exercise = { position, catalogExerciseId, name: String(item.name || "").trim(), muscle: String(item.muscle || "").trim(), sets, reps, weight, technique: String(item.technique || ""), trainingDay: String(item.trainingDay || ""), completed: typeof item.completed === "boolean" ? item.completed : null, actualReps, note: String(item.note || "").trim() };
    if ((catalogExerciseId !== null && catalogExerciseId.length > 100) || !exercise.name || exercise.name.length > 100 || !exercise.muscle || exercise.muscle.length > 60 || !Number.isInteger(sets) || sets < 1 || sets > 20 || !Number.isInteger(reps) || reps < 1 || reps > 100 || !Number.isFinite(weight) || weight < 0 || weight > 1_000 || !techniques.includes(exercise.technique) || !routineDays.includes(exercise.trainingDay) || (actualReps !== null && (!Number.isInteger(actualReps) || actualReps < 0 || actualReps > 100)) || exercise.note.length > 500) return null;
    return exercise;
  });
