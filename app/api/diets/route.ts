...
const summaryIncludeFor = (userId: string) => ({ meals: { orderBy: { position: "asc" as const } }, weightEntries: { where: { userId }, orderBy: { date: "desc" as const }, take: 2 }, dailyLogs: { where: { userId, date: new Date(`${dateKey(new Date())}T12:00:00.000Z`) } }, members: { select: { userId: true } } });
