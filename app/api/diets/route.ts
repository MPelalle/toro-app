...
const summaryIncludeFor = (userId: string) => ({ meals: { orderBy: { position: "asc" as const } }, weightEntries: { where: { userId }, orderBy: { date: "desc" as const }, take: 2 }, dailyLogs: { where: { userId, date: dateAtNoonUTC(appDateKey()) } }, weeklyCheckIns: { where: { userId }, orderBy: { weekStart: "desc" as const }, take: 2 }, members: { select: { userId: true } } });
const accessibleWhere = (userId: string) => ({ OR: [{ userId, kind: "PERSONAL" as const }, { kind: "SHARED" as const, members: { some: { userId } } }] });
function serialize(plan: any, viewerId: string, weightEntriesAreNewestFirst = false) { const weightEntries = weightEntriesAreNewestFirst ? [...plan.weightEntries].reverse() : plan.weightEntries; const weeklyCheckIns = plan.weeklyCheckIns ? (weightEntriesAreNewestFirst ? [...plan.weeklyCheckIns].reverse() : plan.weeklyCheckIns) : []; return { ...plan, createdAt: plan.createdAt.toISOString(), canEdit: plan.userId === viewerId, meals: plan.meals.map((meal: any) => ({ ...meal, foods: Array.isArray(meal.foods) ? meal.foods : [] })), weightHistory: weightEntries.map((item: any) => ({ ...item, date: dateKey(item.date) })), dailyLogs: plan.dailyLogs.map((item: any) => ({ date: dateKey(item.date), completedMeals: Array.isArray(item.completedMealIds) ? item.completedMealIds : [], comment: item.comment || "" })), weeklyCheckIns: weeklyCheckIns.map((item: any) => ({ ...item, weekStart: dateKey(item.weekStart) })) }; }
