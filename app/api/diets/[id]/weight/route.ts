...
export async function POST(request: Request, ctx: RouteContext<"/api/diets/[id]/weight">) {
  ...
  const diet = await getPrisma().dietPlan.findFirst({ where: { id, OR: [{ userId: user.id, kind: "PERSONAL" }, { kind: "SHARED", members: { some: { userId: user.id } } }] } });
  ...
  const entry = await getPrisma().dietWeightEntry.create({ data: { dietId: id, userId: user.id, date: new Date(`${date}T12:00:00.000Z`), weight, note: note || null } });
