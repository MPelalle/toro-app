...
type Dashboard = {
  routine: { id: string; name: string; type: string; days: string[] };
  members: Array<{
    id: string;
    name: string;
...
export default function SharedRoutineDashboardPage() {
  ...
  return <main className="min-h-dvh bg-[#090a08] px-4 pb-36 pt-28 text-white sm:px-8"><div className="mx-auto max-w-4xl">
    ...
    <header className="mt-8 flex flex-wrap items-end justify-between gap-4"><div><p className="text-[10px] font-bold tracking-[.22em] text-[#b7ff00]/70">RUTINA COMPARTIDA</p><h1 className="mt-2 text-3xl font-semibold tracking-[-.05em] sm:text-4xl">{data.routine.name}</h1><p className="mt-2 text-sm text-white/40">{data.routine.type} · {data.routine.days.join(", ")}</p></div><div className="flex flex-wrap gap-2"><Link href={`/dashboard/routine/${id}`} className="inline-flex items-center gap-2 rounded-xl bg-[#b7ff00] px-4 py-3 text-sm font-bold text-black"><Dumbbell size={16}/> Entrenar</Link><Link href={`/dashboard/routine/${id}/edit`} className="inline-flex items-center gap-2 rounded-xl border border-white/10 px-4 py-3 text-sm font-semibold text-white/70"><Pencil size={15}/> Editar plan</Link></div></header>
