...
export default function RoutineDetailPage() {
  ...
  const updateSession = async (patch: Partial<Pick<OfflineWorkoutSession, "notes" | "emotionalRating">>) => {
      ...
      <div className="relative mx-auto max-w-4xl">
        ...
        {!session ? <>
          ...
          <section className="mt-5 rounded-[28px] border border-white/[.08] bg-[#10110e]/95 p-4 sm:p-6">
            ...
            <div className="mt-6 grid gap-4 border-t border-white/[.07] pt-5 sm:grid-cols-2">
              <label className="text-xs text-white/50">Nota del entrenamiento<textarea disabled={session.status === "FINISHED"} value={session.notes ?? ""} onChange={(event) => void updateSession({ notes: event.target.value || null })} className="input mt-2 h-24 resize-none py-3" maxLength={2000} placeholder="Cómo te sentiste, técnica, molestias…" /></label>
              <div><p className="text-xs text-white/50">Valoración emocional</p><div className="mt-2 flex gap-2">{[1, 2, 3, 4, 5].map((rating) => <button key={rating} type="button" disabled={session.status === "FINISHED"} onClick={() => void updateSession({ emotionalRating: rating })} className={`grid h-10 w-10 place-items-center rounded-xl border text-sm ${session.emotionalRating === rating ? "border-[#b7ff00] bg-[#b7ff00] text-black" : "border-white/10 text-white/55"}`}>{rating}</button>)}</div><p className="mt-2 text-[11px] text-white/30">1 difícil · 5 excelente</p></div>
            </div>
            <div className="mt-5 flex justify-end">
              ...
              {session.status === "FINISHED" && <ShareWorkoutButton session={session} workoutName={routine.name} />}
            </div>
          </section>
