...
export default function EditRoutinePage() {
  ...
  const save = async () => {
    ...
    try {
      if (routine.kind === "SHARED") {
        ...
        await updateRoutineOfflineFirst(id, { name: routine.name, type: routine.type, days: routine.days });
      }
  ...
  return <main className="relative min-h-dvh overflow-hidden bg-[#090a08] px-4 pb-36 pt-28 text-white sm:px-8"><div className="relative mx-auto max-w-2xl">
    ...
    <section className="mt-7 rounded-[28px] border border-white/[.08] bg-[#10110e] p-5 sm:p-7">
      ...
      <label className="mt-5 block"><span className="mb-2 block text-xs text-white/55">Tipo de rutina</span><select className="input" value={routine.type} onChange={(event) => updateRoutine({ type: event.target.value })}><option>Weider</option><option>Torso / Pierna</option><option>Fullbody</option></select></label>
